...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-storage-venv
    export POETRY_HOME := /tmp/poetry-storage-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-storage-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/storage-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-storage-cache
endif
